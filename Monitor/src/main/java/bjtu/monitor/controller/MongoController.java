...
//    @ApiOperation("查询文件")
    public void findFileById(String id, HttpServletResponse response) throws Exception {
        GridFSFile gridFSFile = monggoDB.findOneFile(id);
        //打开下载流对象
        if (null != gridFSFile) {
...
//    @ApiOperation("图片上传")
    public String uploadFile(@RequestParam("file") MultipartFile blobFile,@RequestParam("userid")int userid) throws IOException {
        ObjectId id = monggoDB.upload(blobFile,userid);
